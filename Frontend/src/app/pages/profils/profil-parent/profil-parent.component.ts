...
@Component({
  ...
  styleUrls: ['./profil-parent.component.scss']
})
export class ProfilParentComponent implements OnInit {
  ...
  constructor(
    ...
    private adminServiceService :ParentService,
  ...
  onUpdateSuperUserInterne() {
    ...
    if (this.profilFormSuperUserInterne.valid) {
      ...
      const updatedUserInterneTechncien: any = {
        id: this.userConnectedById.parent.id,
    ...
    this.userService.findById(id).subscribe({
      next: (result: UserConnected) => {
        ...
        this.profilFormSuperUserInterne.patchValue({
          ...
          adresse: result.parent?.adresse || '',
          telephone: result.parent?.telephone || '', // Utilisez le ? pour éviter les erreurs si client est null
          codePostal: result.parent?.codePostal || '',
        });
      },

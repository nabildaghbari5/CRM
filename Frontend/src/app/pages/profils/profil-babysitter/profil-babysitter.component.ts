...
import { BabysiterService } from '../service/babysiter.service';

@Component({
  selector: 'app-profil-babysitter',
  templateUrl: './profil-babysitter.component.html',
  styleUrls: ['./profil-babysitter.component.scss']
})
export class ProfilBabysitterComponent implements OnInit {
  ...
  constructor(
    ...
    private adminServiceService :BabysiterService,
  ...
  onUpdateSuperUserInterne() {
    ...
    if (this.profilFormSuperUserInterne.valid) {
      ...
      const updatedUserInterneTechncien: any = {
        id: this.userConnectedById.babysitter.id,
    ...
    this.userService.findById(id).subscribe({
      next: (result: UserConnected) => {
        ...
        this.profilFormSuperUserInterne.patchValue({
          ...
          adresse: result.babysitter?.adresse || '',
          telephone: result.babysitter?.telephone || '', // Utilisez le ? pour éviter les erreurs si client est null
          codePostal: result.babysitter?.codePostal || '',
        });
      },
